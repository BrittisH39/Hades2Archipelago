...
Bridge.on("ITEMS", function(payload)
  ItemManager.apply_full_list(payload)
  -- Apply the run-start fillers the moment the item counts we need are actually available,
  -- instead of only on the next room-entry/room-clear hook. The AP bridge connects and delivers
  -- this asynchronously, so on a fresh launch a room can load BEFORE this fires -- those hooks
  -- reassert too (see reload.lua's StartRoom/RestoreUnlockRoomExits/UnlockRoomExits wraps and
  -- ItemManager.reassert_stat_bonuses' own comment), but this makes the correction immediate
  -- instead of waiting for the next room transition or room clear.
...
if not AP_hooks_installed then
  ...
  -- early Oath of the Unseen, the early field-NPC "helpers", early Chaos Gates, and the early
  -- Fated List. IsGameStateEligible (RequirementsLogic.lua:9) is the game's single gate for "is
  -- this content available right now" -- the Crossroads Oath obelisk (DeathLoopData Shrine
  -- object, gated NamedRequirements ShrineUnlocked, DestroyIfNotSetup), the Chaos Gate rooms
  -- (RoomData/RoomDataI/N/P, gated NamedRequirements {"ChaosUnlocked","NoRecentChaosEncounter"}),
  -- the Crossroads QuestLog/Fated List pedestal (HubRoomData.Hub_Main.ObstacleData[560662]), and
  -- each helper's intro encounter all flow through it.
  ...
  -- npc_locations: Eris' Underworld "Curse" ambush (RoomDataG/H/I.lua SpawnErisForCurse ->
  -- NPC_Eris_01, a scripted spawn+dialogue, not a normal StartEncounter). Her curse dialogue
  -- ends by calling ApplyErisCurse (ShrineLogic.lua) regardless of how the conversation was
  -- entered, so it's a more reliable "you've met her" signal than relying on UseNPC alone for
  -- this specific spawn path. on_eris_curse_applied only sends when Surface is part of the
  -- seed (Met Eris stays Surface-gated); harmless no-op otherwise.
  pcall(function()
  ...
  pcall(function()
    modutil.mod.Path.Wrap("OpenRunClearScreen", function(base, ...)
      ...
      pcall(function()
        ...
        -- final boss room's biome, and the equipped weapon for the goal count. This is
        -- banked UNCONDITIONALLY, before anything about Final Challenge below -- a later
        -- Zagreus win/loss must never undo Chronos/Typhon's own AP credit.
      ...
      if redirect_to_zagreus then
        -- Final Challenge: skip the vanilla run-clear screen entirely and drop the player
        -- straight into the secret Zagreus fight instead, reusing the exact room-creation
        -- call the game's own vanilla "ZagContract" secret door uses (EventLogic.lua
        -- SpawnZagContract: CreateRoom(RoomData.C_Boss01) then a transition into it). We
        -- call LeaveRoom directly rather than spawning a physical door obstacle, since
        -- Chronos'/Typhon's boss rooms have no authored door-placement point to spawn one
        -- at; the Hero's own ObjectId stands in as the position anchor LeaveRoom uses only
        -- for the cosmetic exit-direction/transition-wipe calculation. Best-effort: this is
        -- the one piece of the feature that needs in-game verification (see project plan).
        local handled = false
        pcall(function()
          local currentRun = game.CurrentRun
          local roomData = game.RoomData and game.RoomData.C_Boss01
          if currentRun and currentRun.Hero and roomData and game.CreateRoom and game.LeaveRoom then
            local nextRoom = game.CreateRoom(roomData)
            game.LeaveRoom(currentRun, { ObjectId = currentRun.Hero.ObjectId, Room = nextRoom })
            handled = true
            rom.log.info("[AP] Final Challenge: redirected into the Zagreus fight")
          end
        end)
        if handled then return end
        rom.log.warning("[AP] Final Challenge: Zagreus redirect failed, falling back to the run-clear screen")
      end
      return base(...)
    end)
  end)
