...
-- Goal ids (match Options.Goal / Routes.GOAL_DEFS) whose boss set includes Zagreus.
local ZAGREUS_GOAL_IDS = { [2] = true, [3] = true, [4] = true, [6] = true, [7] = true }
...
function ItemManager.goal_includes_zagreus()
  return ZAGREUS_GOAL_IDS[ItemManager.setting_mode("goal")] == true
end
...
local function resolve_eligibility_tables()
  ...
  if not (nrd and ed and hrd) then return false end
  local force = {}
...
--              Fated List pedestal + QuestLogUnlocked)
...
function ItemManager.eligibility_override(requirements)
  ...
  -- Zagreus Vanilla mode: force the secret superboss's real unlock gate
  -- (StoreData.ZagreusContractRequirement / InfernalContractUnlocked, normally "have reached
  -- the true ending once") true, so the fight can appear from run 1. Only when Zagreus is
  -- actually part of the goal and Vanilla is the chosen encounter mode -- everything else
  -- about his vanilla spawn mechanics (which zones/rooms) is left completely untouched.
  local storeData = game.StoreData
  if storeData and requirements == storeData.ZagreusContractRequirement
     and ItemManager.goal_includes_zagreus()
     and ItemManager.zagreus_mode() == ItemManager.ZAGREUS_MODE_VANILLA then
    return "true"
  end
  return nil
end
