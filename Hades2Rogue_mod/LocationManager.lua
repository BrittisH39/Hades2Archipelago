...
-- Mirrors Python Locations.SHARED_ENEMY_ZONES: these checks exist when EITHER of their
-- routes is in the seed, and either route's kill satisfies them.
...
LocationManager.ENEMY_UNIT_OVERRIDE = LocationManager.ENEMY_UNIT_OVERRIDE or {
  ...
  ["HadesBloodlessGrenadierElite"] = "Dire Inferno-Bombers", -- own check: the Tartarus MiniBossGrenadier pair ("Dire Inferno-Bomber")
  ...
  ["SpreadShotUnitMiniboss"] = "Dire Spreaders", -- own check: Asphodel MiniBossSpreadShot ("Dire Spreader")
...
-- "Met Megaera" (zone 1) has no location on the Python side right now (her keepsake-giver
-- "Met" and the 3-random-Furies boss fight are both temporarily unlocated -- see Python
-- NPC_NO_MEET) -- kept here anyway so the mod still fires the send; the client just shows
-- "Unknown location" until this is revisited. Zone 4's "Met Hades" is deliberately the SAME
...
-- "<NPC> Keepsake" locations only exist in the seed when that route is active. "Hades" is
-- NOT here -- he's met via either route (see NPC_MULTI_ROUTE_HADES / Python
-- Routes.NPC_MULTI_ROUTE_LOCK) -- and Megaera/Thanatos/Orpheus/Achilles stay Nightmare-gated
-- even though their locations don't exist right now (Python NPC_NO_MEET/KEEPSAKE_NO_LOCATION):
-- this just stops an attempted send on a seed where Nightmare content isn't even loaded.
local NPC_ROUTE_LOCK = {
  ...
  Sisyphus = "Nightmare", Eurydice = "Nightmare", Patroclus = "Nightmare", Orpheus = "Nightmare",
  Megaera = "Nightmare", Thanatos = "Nightmare", Achilles = "Nightmare",
}

-- "Met Hades" is reachable via either route (mirrors Python Routes.NPC_MULTI_ROUTE_LOCK) --
-- gated below in ROUTE_GATED_CHECKS as "any of these routes", not a single required one.
local NPC_MULTI_ROUTE_HADES = { "Underworld", "Nightmare" }
...
for _, _name in ipairs(SHARED_ENEMY_CHECKS) do
  ROUTE_GATED_CHECKS[_name .. " Defeated"] = { "Underworld", "Nightmare" }
end
...
-- Overrides the single-route gate the BOSS_MET_BY_ZONE loop above set for "Met Hades"
-- (Nightmare only): it's reachable via either route, so it's blocked only when NEITHER is
-- active this seed.
ROUTE_GATED_CHECKS["Met Hades"] = NPC_MULTI_ROUTE_HADES
...
local NPC_CAST = {
  ...
  -- instead, same convention as Hecate/Eris not being double-listed here).
...
-- game's own signal for "this encounter is done and something popped". Dedup'd per encounter
-- instance (encounter._ap_scored) so a re-entrant call is a no-op, same pattern as room._ap_scored.
...
-- (YAML chronos_defeats_needed / typhon_defeats_needed / hades_defeats_needed). Defaults to
-- 1 if unset.
...
local function defeats_needed(route)
  local key = DEFEATS_NEEDED_KEY[route] or "chronos_defeats_needed"
...
-- times (chronos_defeats_needed / typhon_defeats_needed, default 1), the player has
...
-- enemy RNG spawns either. Shared-zone enemies (SHARED_ENEMY_CHECKS) are included when route
-- is one of their owning routes (Underworld/Nightmare), matching how ROUTE_GATED_CHECKS already
-- treats those names as belonging to either route. send_first (existing dedup + route-block
-- guard) does the actual work; this just walks every name the route owns.
local function flush_route_enemy_checks(route)
  ...
  if route == "Underworld" or route == "Nightmare" then
