...
-- (YAML chronos_defeats_needed / typhon_defeats_needed). Defaults to 1 if unset.
local function defeats_needed(route)
  local key = (route == "Surface") and "typhon_defeats_needed" or "chronos_defeats_needed"
...
local function route_clears(s, route)
  return (route == "Surface") and s.typhon_clears or s.chronos_clears
end
...
local function flush_route_checks(route, pool)
  ...
  if system == 0 then
    ...
    local sent = 0
    while pool.next_check <= limit do
      if not ItemManager.is_score_checked(route, pool.next_check) then
        local check = Routes.SCORE_PREFIX[route] .. " " .. pad4(pool.next_check)
    ...
    rom.log.info("[AP] goal cascade: flushed " .. sent .. " " .. route .. " score checks (up to " .. limit .. ")")
    return
  end
  local combine = ItemManager.combine_active()
  local limit = room_limit(route)
...
-- Send the goal state: Chronos clears / distinct weapons, Typhon clears / weapons, then
-- Zagreus clears (no weapon-variety requirement for Zagreus -- see Client.py evaluate_goal).
