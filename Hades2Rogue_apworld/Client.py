...
class Hades2CommandProcessor(ClientCommandProcessor):
    ...
    def _cmd_bridge(self):
        ...
        connected = self.ctx.bridge_writer is not None
        logger.info(f"Game bridge connected: {connected} (listening on {BRIDGE_HOST}:{BRIDGE_PORT})")
...
class Hades2Context(CommonContext):
    ...
    def on_package(self, cmd: str, args: dict) -> None:
        if cmd == "Connected":
            ...
            if self.slot_data.get("deathlink"):
            ...
            if "tags" in args and "DeathLink" in args["tags"]:
    ...
    def compute_score_sync(self) -> str:
            ...
            if name.startswith("Room "):
                ...
                if " " not in remainder:   # weapon suffix -> per-weapon combined, skip
                    try:
                        ...
                        pass
                continue
            for prefix, is_underworld in (
                ("Underworld Room ", True), ("Surface Room ", False),
            ):
        ...
        return (f"SCORESYNC:underworld_room={uroom};surface_room={sroom};"
                f"combined_room={croom}")

    def compute_checked_score(self) -> str:
        # point_based only: tell the mod which "<route> Score N" checks the server ALREADY
        # has (a finished player's auto-released/collected checks, an admin !send_location,
        # fresh-save recovery). The mod advances past these for FREE - no score spent, no
        # CHECK re-sent. This is per-number (not a high-water mark) so a gap like "0017
        # checked while 0015 is not" is handled correctly. Parsing mirrors compute_score_sync
        # (names with a trailing space + weapon suffix are room checks and are skipped).
        id_to_name = {loc_id: name for name, loc_id in self.location_name_to_id.items()}
        underworld, surface = [], []
        for loc_id in self.checked_locations:
            name = id_to_name.get(loc_id, "")
    ...
    def encode_settings(self) -> str:
        ...
        keys = [
            ...
            "goal", "zagreus_encounter_mode", "included_routes", "separate_checks",
            "starting_route", "lock_routes",
            "underworld_offset", "surface_offset", "surface_start", "start_with_surface_cure",
            "underworld_active", "surface_active",
            "chronos_defeats_needed", "typhon_defeats_needed", "zagreus_defeats_needed",
            "zagreus_weaken_tiers", "weapons_clears_needed",
            "ashes_pack_value", "psyche_pack_value", "bones_pack_value",
    ...
    # Goal id -> ("and"/"or", {boss keys}). Mirrors Routes.GOAL_DEFS; kept as an inline
    # duplicate since this module runs standalone and doesn't import the apworld package.
    GOAL_DEFS = {
        0: ("and", {"chronos"}),
        1: ("and", {"typhon"}),
        2: ("and", {"zagreus"}),
        3: ("and", {"chronos", "zagreus"}),
        4: ("and", {"typhon", "zagreus"}),
        5: ("and", {"chronos", "typhon"}),
        6: ("and", {"chronos", "typhon", "zagreus"}),
        7: ("or", {"chronos", "typhon", "zagreus"}),
    }

    def evaluate_goal(self, payload: str) -> None:
        # payload: "<chronos_clears>-<chronos_weapons>-<typhon_clears>-<typhon_weapons>-<zagreus_clears>"
        if not self.slot_data:
            return
        parts = payload.split("-")
        try:
            chronos_clears, chronos_weapons, typhon_clears, typhon_weapons, zagreus_clears = \
                (int(p) for p in parts[:5])
        except (ValueError, IndexError):
            logger.warning(f"Malformed VICTORY payload: {payload!r}")
            return

        weapons_needed = int(self.slot_data.get("weapons_clears_needed", 1))
        chronos_ok = (chronos_clears >= int(self.slot_data.get("chronos_defeats_needed", 1))
                      and chronos_weapons >= weapons_needed)
        typhon_ok = (typhon_clears >= int(self.slot_data.get("typhon_defeats_needed", 1))
                     and typhon_weapons >= weapons_needed)
        # No weapon-variety requirement for Zagreus.
        zagreus_ok = zagreus_clears >= int(self.slot_data.get("zagreus_defeats_needed", 1))
        achieved = {"chronos": chronos_ok, "typhon": typhon_ok, "zagreus": zagreus_ok}

        mode, bosses = self.GOAL_DEFS.get(int(self.slot_data.get("goal", 7)), self.GOAL_DEFS[7])
        done = any(achieved[b] for b in bosses) if mode == "or" else all(achieved[b] for b in bosses)
