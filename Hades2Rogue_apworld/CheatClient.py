...
class Hades2CheatContext(Hades2Context):
    ...
    def make_gui(self):
        ...
        # it's what pulls in kvui (which imports "kivy" itself), and some kvui builds
        # (e.g. one vendored inside another installed world) assert that kvui is the
        # first thing to import kivy, hard-crashing otherwise. See Client.py's make_gui
        # for the full story -- this is the same mistake, fixed the same way.
        ui = super().make_gui()
...
def launch():
    # See Client.py's launch() -- same reasoning: without this, a crash here is
    # completely invisible (no console, nothing in Archipelago's own logs folder).
    Utils.init_logging("Hades2RogueCheatClient")
    ...
    try:
        ...
        logger.exception("Hades 2 Rogue Client (Cheats) crashed on startup")
        Utils.messagebox(
            "Hades 2 Rogue Client (Cheats) Error",
            "The Hades 2 Rogue Client (Cheats) crashed on startup.\n\n"
            "Please check logs/Hades2RogueCheatClient.txt in your Archipelago folder.",
            error=True,
        )
    finally:
        colorama.deinit()
