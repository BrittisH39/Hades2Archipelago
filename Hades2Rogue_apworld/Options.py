...
class Goal(Choice):
    """Which final boss(es) you must defeat to win. Zagreus is the game's secret
    superboss fight -- see Zagreus Encounter Mode for how/when it becomes available."""
    display_name = "Goal"
    option_chronos = 0
    option_typhon = 1
    option_zagreus = 2
    option_chronos_and_zagreus = 3
    option_typhon_and_zagreus = 4
    option_chronos_and_typhon = 5
    option_all_three = 6
    option_any_of_three = 7
    default = 7
...
class ZagreusEncounterMode(Choice):
    ...
    empowered. Progressive Zagreus Weaken items are added to the multiworld that weaken
    him back down to, and eventually below, his normal strength.
    Final Challenge: Defeating Chronos or Typhon immediately drops you into the Zagreus
    fight instead of ending the run."""
...
class ZagreusDefeatsNeeded(Range):
    ...
    Ignored if Zagreus is not part of your goal.
    """
...
class ZagreusWeakenTiers(Range):
    """
    How many "Progressive Zagreus Weaken" items exist to bring an Empowered Zagreus back
    down (and eventually below) his normal strength. Ignored unless Zagreus Encounter Mode
    is Empowered.
    """
...
class ChronosDefeatsNeeded(Range):
    ...
    Note: with the "Any of the 3" goal you only need to reach this count OR the Typhon/Zagreus
    count (whichever boss you actually beat), not all of them.
    """
...
class TyphonDefeatsNeeded(Range):
    ...
    Note: with the "Any of the 3" goal you only need to reach this count OR the Chronos/Zagreus
    count (whichever boss you actually beat), not all of them.
    """
