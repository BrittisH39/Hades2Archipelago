...
# --- Boss-tier gates (weapons / grasp / arcana / vow weight / progressive weapons / gods) ---
...
# percentage of whichever resource pools are actually active this seed: 20% for the 1st boss,
# 40% the 2nd, 60% the 3rd, 75% the 4th (final). Grasp is deliberately percentage-of-cap
# (rather than a flat count) since players can freely change how many Progressive Grasp items
# exist (GraspCount); Arcana/vow-weight/progressive-weapon/gods-unlocked share the same
# percentages for consistency. GodSanity (added July 21) is a resource pool the same way: while
# it's active, boons from a locked god can never be picked up, so a boss tier assuming you've
# accumulated boon-derived power needs its own share of the 11 "<God> Unlock" items too, not
# just grasp/arcana -- otherwise a seed could demand 75% arcana off boon-fueled runs while
# GodSanity still had 10 of 11 gods locked. Each piece only gates when its mode is actually in
# play this seed (mirrors _hades2_grasp_count/_hades2_arcana_count's "return a large number
# when off" pass-through pattern). Trimmed July 17 (generation-failure pass) from
# [0.25, 0.50, 0.75, 0.90]: combined with _tier_requirement_met's old AND-of-every-active-pool
# check, a 90% final tier forced nearly the full item pool into an early sphere on seeds with
# several pools active at once. Same increasing shape, lower ceiling.
BOSS_TIER_PERCENT = [0.20, 0.40, 0.60, 0.75]
...
def _tier_requirement_met(state, player: int, options, percent: float, grasp_cap: int,
    ...
    """Whether `state` clears the given tier's percentage across MOST (not all) of the resource
    pools that are actually active this seed (grasp/arcana/progressive-weapon-count/vow-weight-
    removed/gods-unlocked). Shared by every boss's own tier gate and by the Zagreus checks
    (see set_rules).

    Loosened July 17 (generation-failure pass): this used to be a strict AND across every
    active pool. On seeds with several systems active at once (e.g. grasp + progressive_arcana
    + reverse_vow + progressive weapons, all common/default-weighted options),
    that forced a single state to satisfy every pool simultaneously at each tier -- by the 75-90%
    final tier that's effectively "hold almost the entire item pool," which the fill algorithm
    can't satisfy for the handful of locations that need to be reachable that early. Now a tier
    only needs a STRICT MAJORITY (more than half) of whichever pools are active
    (len(checks)//2 + 1): 2 of 3, 3 of 4, 3 of 5, 4 of 6. Keeps real cross-resource gating (you
    can't ignore every pool but one) without demanding simultaneous completion of all of them.
    Tightened July 21 when GodSanity joined the pool (grasp/arcana/vow/weapon/god can now reach
    5 active pools at once): the previous "at least half" (ceil(active/2)) let an even pool
    count pass on an exact tie (e.g. 2 of 4), which stopped feeling like a real majority once a
    5th pool was possible -- "more than half" is unambiguous at every pool count."""
    checks = []
    if grasp_cap < 99:
    ...
    if not checks:
        return True
    return sum(checks) >= len(checks) // 2 + 1
...
def set_rules(world: "Hades2World", player: int, options, route_offsets: dict,
    ...
    # Zagreus" goal event above. Met opens up as soon as ANY route's 1st ("tier 1", 25%) boss
    # is beatable -- an early, cheap gate, since this is meant to just be a first sighting.
    # EXCEPT in Final Challenge mode (user request, July 22): there the Zagreus contract only
    # ever spawns after defeating a route's FINAL boss (see boss_event("Zagreus") above / the
    # ItemManager hook), so "meeting" him can't happen any earlier than that in that mode --
    # gate on any route's final-tier boss being beatable instead of its 1st-tier one.
    # Defeated is deliberately harder than even the 4th/final tier (90%): it needs the FULL
    # (100%) share of every active resource pool, all 6 weapons, any route's final zone, and
    # (in Empowered mode) the full Weaken-tier count rather than the 60% the goal event uses.
