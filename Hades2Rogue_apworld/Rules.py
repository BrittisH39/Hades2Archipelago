...
from .Routes import ROUTES, UNDERWORLD, SURFACE, boss_event, boss_victory, active_routes, \
    NPC_ROUTE_LOCK, GOAL_DEFS
...
# Met-only hard gates: NPCs with no Keepsake check (their keepsake is in
# KEEPSAKE_NO_LOCATION) but whose "Met <NPC>" location still needs a route/area gate.
# Hades is only encountered deep in the Underworld (Known Bugs), so his Met check needs
# the final Underworld layer -- same zone as "Met Chronos".
MET_ONLY_HARD = {
    "Hades": (NPC_ROUTE_LOCK["Hades"], 3, 0),
}
    ...
    def _hades2_can_get_victory(self, player: int, options) -> bool:
        ...
        achieved = {
            ...
            "typhon": self.has("Typhon Victory", player)
                      and self._hades2_has_enough_weapons(player, options, weapons),
            # No weapon-variety requirement for Zagreus -- a single clear with any weapon
            # satisfies the goal's Zagreus part.
            "zagreus": self.has("Zagreus Victory", player),
        }
        mode, bosses = GOAL_DEFS[options.goal.value]
        if mode == "or":
...
def set_rules(world: "Hades2World", player: int, options, route_offsets: dict,
    ...
    # chain. Reachable once zone index 1 of ANY active route is reachable -- the real game's
    # secret "ZagContract" door only spawns in specific zones (confirmed: Underworld zones
    # 1-2, Surface zones 2-3), and zone index 1 is the earliest zone common to both routes.
    zagreus_zones = [ROUTES[route]["zones"][1] for route in routes]
    add_rule(world.get_location(boss_event("Zagreus"), player),
             lambda state, zones=zagreus_zones:
...
def _set_keepsake_rules(world: "Hades2World", player: int, options, routes: list) -> None:
    ...
    if keepsakes_active:
        ...
        for location in npc_locations("Moros"):
            add_rule(location, lambda state: state.has(KEEPSAKE_MOROS_ITEM, player))

    # Hard keepsakes/Met-only NPCs: reach the route's area (when that route is in the seed)
    # AND own enough keepsakes. These gate "Met <NPC>" even when keepsakesanity is off,
    # since that's what stops off-route NPCs (Icarus, Dionysus, Arachne, ...) from being
    # unconditionally reachable.
    for npc, (route, area, threshold) in {**KEEPSAKE_HARD, **MET_ONLY_HARD}.items():
