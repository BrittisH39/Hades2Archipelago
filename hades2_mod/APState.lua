...
local DEFAULTS = {
  ...
  grasp_count = 0,      -- number of Progressive Grasp items received (graspsanity)
  ...
  arachne_armor = 0,    -- Arachne Armor filler items received (random armor at run start)
...
local ROUTES = { "Underworld", "Surface" }
...
function APState.get()
  ...
  if s.combined_weapon == nil then s.combined_weapon = {} end -- weapon short-name -> Progressive <Weapon> count (weapon_aspect_combine)
